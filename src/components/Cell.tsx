...
const getCellContent = (state: CellState): React.ReactNode => {
  if (state.isFlagged) return <Flag className="w-4 h-4 text-white" />;
  if (!state.isRevealed) return null;
  if (state.isMine) return <Bomb className="w-5 h-5 text-white" />;
  if (state.adjacentMines > 0) return state.adjacentMines;
  return null;
};

const getCellStyles = (state: CellState, clickable: boolean): string => {
  let baseStyle = 'w-9 h-9 md:w-10 md:h-10 flex items-center justify-center font-bold text-lg rounded border';
